...
acf(usconsumption[,1])
pacf(usconsumption[,1]) # moving average?
...
plot(forecast(usconsumption[,1])) #holt.winters
plot(forecast(fit))#arima model forecast
...
plot(forecast(log(AirPassengers)))
plot(forecast(fit))
...
summary(fit) # much better AIC and exp(RMSE) to compare the RMSE with the prev fit
